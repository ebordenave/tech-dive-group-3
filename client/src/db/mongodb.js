...
async function createExams(client, exams) {
  ...
  console.log(
    `${result.insertedCount} new exams created with the following id(s):`
  );
...
async function getByExam(client, exam) {
  const result = await client
    .db("exams")
    .collection("exam")
    .findOne({ patientId: exam.patientId });

  if (result) {
    console.log(
      `Found an exam in the collection with the patientId: '${result.patientId}'`
    );
  } else {
...
async function updateExam(client, exam, members) {
  // members: { <field> : <value>, ... }
  const result = await client
    .db("exams")
    .collection("exam")
    .updateOne(exam, { $set: members });
